      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
                {loading ? (
                <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-500 font-medium">Đang tải dữ liệu...</td></tr>
                ) : users.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-500 font-medium">Không có dữ liệu người dùng</td></tr>
                ) : (
                users.map((user) => {
                    const roleInfo = ROLE_LABELS[user.role] ?? { text: user.role, cls: 'bg-gray-100 text-gray-700 border-gray-200' };
                    return (
                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-4">
                            <div className="flex items-center">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-purple-100 to-indigo-100 flex items-center justify-center text-indigo-700 font-bold mr-4 border border-indigo-200 shadow-sm">
                                    {user.fullName?.charAt(0) || user.username?.charAt(0) || '?'}
                                </div>
                                <div>
                                    <div className="text-sm font-bold text-gray-900">{user.fullName}</div>
                                    <div className="text-xs font-medium text-gray-500 mt-0.5">@{user.username} • {user.email}</div>
                                </div>
                            </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`px-2.5 py-1 inline-flex text-xs font-bold rounded-full border ${roleInfo.cls}`}>
                            {roleInfo.text}
                        </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {user.active ? (
                            <span className="px-2.5 py-1 inline-flex text-xs font-bold rounded-full bg-green-50 border border-green-200 text-green-700">
                            Hoạt động
                            </span>
                        ) : (
                            <span className="px-2.5 py-1 inline-flex text-xs font-bold rounded-full bg-red-50 border border-red-200 text-red-700">
                            Đã khóa
                            </span>
                        )}
                        </td>
                        ...
                        {user.role !== 'SYSTEM_ADMIN' && (
                            ...
                            className={`inline-flex items-center px-3 py-1.5 border rounded-lg text-xs font-bold transition-colors ${
                                user.active 
                                ? 'border-red-200 text-red-700 bg-red-50 hover:bg-red-100' 
                                : 'border-green-200 text-green-700 bg-green-50 hover:bg-green-100'
                            }`}
                            >
                            {user.active ? <><Lock className="w-3.5 h-3.5 mr-1.5" /> Khóa tài khoản</> : <><Unlock className="w-3.5 h-3.5 mr-1.5" /> Mở khóa</>}
                            </button>
                        )}
                        </td>
                    </tr>
